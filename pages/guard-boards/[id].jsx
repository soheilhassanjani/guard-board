...
const AddGuardBoardsPage = () => {
  ...
  const getGuardBoardById = useGetGuardBoardById(query.id ?? null);
  const putUpdateGuardBoard = usePutUpdateGuardBoard();
  //
  ...
  //
  useEffect(() => {
    if (getGuardBoardById.data) {
      const { guardBoardDate, directionCount, guardCount, soldiers } =
        getGuardBoardById.data;

      setGuardBoardDate(guardBoardDate);
      setDirectionCount(directionCount);
      setGuardCount(guardCount);
      setAddedSoldiers(soldiers);
    }
  }, [getGuardBoardById.data]);
  //
  return (
